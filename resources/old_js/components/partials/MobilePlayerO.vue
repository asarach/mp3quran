<template>
  <div class="mobile-player">
    <div class="sply-progress" :style="'width: ' + progress * 100 +'%;'"></div>
    ...
    <div class="ply-fullplayer" :class="{opened: show_fullplayer}">
      ...
      <div class="fply-body">
        <div class="fply-timer">{{ timer }}</div>
        <div class="fply-progress">
          <vue-slider v-model="progress_bar" tooltip="none" />
        </div>
        <div class="fply-duration">{{ formatTime(duration)}}</div>
      </div>
      <div class="fply-controls">
        <div class="ply-btn btn-previous" :class="{disabled: currentPosition() < 1}">
          <skip-previous-icon @click="prev" fillColor="#fff" />
        </div>
        <div class="ply-btn btn-play-pause" @click="togglePlayback">
          <pause-icon v-if="playing" fillColor="#fff" />
          <play-arrow-icon v-else fillColor="#fff" />
        </div>
        <div class="ply-btn btn-next" :class="{disabled: currentPosition() >= playlist.length - 1}">
          <skip-next-icon @click="next" fillColor="#fff" />
        </div>
      </div>
      <div class="fply-footer">
        <ul class="list-unstyled">
          <li
            v-for="audio in playlist"
            :key="audio.id"
            :class="{playing: isPlaying(audio)}"
          ...
          >
            ...
            <div class="read-options">
              ...
              <div class="ply-btn read-like-btn" v-else @click="addSoraFavorite(audio.هي)">
                <heart-outline-icon fillColor="#fff" />
              </div>
            </div>
          </li>
        </ul>
      ...
      <div class="fply-close" @click="closeFullplayer">
        <keyboard-arrow-down-icon fillColor="#fff" />
      </div>
    </div>
    <div class="ply-playlist" :class="{opened: show_playlist}">
      ...
      <ul class="list-unstyled">
        <draggable v-model="playlist" handle=".drag-handle">
          <li v-for="audio in playlist" :key="audio.id" :class="{playing: isPlaying(audio)}">
