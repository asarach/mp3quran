    ...
    actions: {
        setData({ commit, state }, vm) {
            if (vm.initial) {
                ...
                axios.get(vm.url).then(function (response) {
                    commit('setMushafPages', { mushaf_pages: response.data.mushaf_pages })
        ...
        setAlkahf({ commit, state }, vm) {
            if (vm.initial) {
                ...
                axios.get(vm.url).then(function (response) {
                    commit('setAlkahfPages', { alkahf_pages: response.data.alkahf_pages })
                    commit('setReadSoar', { read_soar: response.data.read_soar })
                    commit('setPage', { page: response.data.page })
