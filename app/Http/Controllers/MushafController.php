...
class MushafController extends Controller
{
    ...
    public function mushaf()
    {
        ...
        $path = request()->path();
        $seo_page = $this->page->model->where('name', 'mushaf')->where('status', 1)->firstOrFail();
        ...
        $data = compact('page', 'parts', 'path', 'page_title');
    ...
    public function alkahf()
    {
        $alkahf_pages = [null];
        for ($i = 294; $i < 306; $i++) {
            $alkahf_pages[] = 'https://cdn.qurango.net/Sura2/files/mobile/' . $i . '.jpg';
        }
        ...
        $read_soar = Cache::rememberForever('alkahf_read_soar_' . LaravelLocalization::getCurrentLocale(), function () {
            ...
            foreach ($reads as $read) {
                ...
                if ($kahf) {
                    $read_soar[] = [
                        'id' =>  '018-' . $read->id,
                        'file' => $read->getAudioUrl(18),
                        'sora_duration' => $kahf->pivot->duration,
                        'sora_id' => $kahf->id,
                        'sora_name' => $kahf->getLocaleName(),
                        'sora_num' => $kahf->getNum(),
                        'reciter_name' => $read->reciter->getLocaleName(),
                        'read_slug' => $read->slug,

                        'share_description' => $read->getLocaleShareDescription($kahf->getLocaleName()),
                        'share_title' => $read->getLocaleShareTitle($kahf->getLocaleName()),
        ...
        $data = compact('page', 'alkahf_pages', 'read_soar', 'page', 'page_share',  'path', 'page_title');
