...
namespace App\Http\Livewire\SpecialRewayat;
...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        if ($this->query) {
            ...
            $cache_name = 'special-rewayat_reads_index_rewaya' . $this->rewaya . '_type_' . $this->type . '_long_' .  LaravelLocalization::getCurrentLocale();
        ...
        return view('livewire.special-rewayat.index', $data);
    }
    ...
    public function getReads()
    {
        $reads = Read::where('status', 1)
            ...
            ->select('id','title', 'special_rewaya_id', 'slug', 'reciter_id');

        if ($this->rewaya) {
            $reads =  $reads->where('special_rewaya_id', $this->rewaya);
        }
